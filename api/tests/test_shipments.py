...
class TestClass(unittest.TestCase):
    ...
    def test_get_shipments(self):
        response = requests.get(url=(self.url + "/shipments"), headers=self.headers)

        self.assertEqual(response.status_code, 200)
    
    def test_get_shipments_by_id(self):
        response = requests.get(url=(self.url + "/shipments/1"), headers=self.headers)

        self.assertEqual(response.status_code, 200)
    
    def test_get_shipments_by_id_items(self):
        response = requests.get(url=(self.url + "/shipments/1/orders"), headers=self.headers)

        self.assertEqual(response.status_code, 200)

    def test_get_shipments_by_id_orders(self):
        response = requests.get(url=(self.url + "/shipments/1/items"), headers=self.headers)
    ...
    def test_post_shipment(self):
        ...
        response = requests.post(url=(self.url + "/shipments"), headers=self.headers, json=data)

        self.assertEqual(response.status_code, 201)

    def test_put_shipment_by_id_(self):
        data = {
        "id": 9999,
        "order_id": 9999,
        "source_id": 9999,
        ...
        response = requests.put(url=(self.url + "/shipments/9999"), headers=self.headers, json=data)
    def test_put_shipment_by_id_(self):
        data = {
        "id": "9999",
        "order_id": 1,
